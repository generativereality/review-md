#!/usr/bin/env node
/**
 * render-doc — markdown → a single self-contained styled HTML file.
 *
 *   render-doc docs/PLAN.md
 *   render-doc docs/PLAN.md --banner-left "Planning · Fri 2026-08-07"
 *   render-doc --manifest packs/planning.json
 *
 * Output is CSS-inlined and font-inlined: it opens from a `file://` URL, offline, on someone
 * else's laptop, with no build step. See `skills/render-doc/SKILL.md` for the design contract.
 *
...
const USAGE = `
render-doc — markdown → one self-contained styled HTML file
...
function renderOne(
  ...
  const html = page({
    ...
    footer:
      ...
      `Rendered from ${sourceLink(repoUrl, repoRelative)} with <code>render-doc</code>. The markdown is the source of truth — edit it there and re-render; don't hand-edit this file.`,
    ...
    renderOne(
      ...
      { ...globals, sectionNumbers: false },
      {
        ...
        provenance: {
          ...
          footer: `Pack cover generated by <code>render-doc --manifest ${escapeHtml(displayPath(root, manifestPath))}</code>. Re-run it after editing any source doc.`,
        },
      },
    );
  } else {
